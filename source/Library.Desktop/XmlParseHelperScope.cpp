...
namespace Library
{
	...
	bool XmlParseHelperScope::startElementHandler(XmlParseMaster::SharedData& sharedData, const std::string& element, const HashMap<std::string, std::string> attributes)
	{
		...
		else if (element == "String")
		{
			...
			datum.set(attributes.find(element)->second);
		}
